...
pb_starts = 0.475 # approximate point of convergence
capT = 50_000
...
# rorsamp = vec(ror_paths[5000:end, :])
# writedlm(solnpath * "rorsamp.txt", rorsamp)

# kd = kde(rorsamp)
# rspace = range(1.095, 1.101, length = 1_000)
# plot(rspace, x -> pdf(kd, x),
#     legend = false,
#     xlabel = L"\bar{p}\theta\hat(\theta, \bar{p})")
# savefig(figpath * "ror_dist.png")

# # RoRs have a fat tail
# xm = minimum(rorsamp) 
# ahat_ror = length(rorsamp) / sum(log.(rorsamp) .- log(xm))
# Rors always below R--how can this be? 

wsamp = vec(w_paths[5000:end, :])
writedlm(figpath * "wsamp.txt", wsamp)

kd_w = kde(wsamp)
wspace = range(minimum(wsamp) - 0.1, maximum(wsamp) + 0.1, length = 5_000)

# this is not a power law
display(plot(wspace, x -> pdf(kd_w, x),
    title = "PDF of w(θ, p̄)",
    legend = false,
    xlabel = L"w(\theta, \bar{p})"))
savefig(figpath * "w_dist.png")

wpdf = map(x -> pdf(kd_w, x), wspace)
writedlm(figpath * "wpdf.txt", wpdf)
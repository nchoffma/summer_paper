...
# # λ_1 = 1.9491874662622317 # works with this, which is bizarre
...
function par_inv(F)
    # Inverse of Pareto CDF for P(xm, α)
...
function fem_newton(Y; ω = 1.0)
    ...
    while diff > tol && its < mxit
        ...
        # b0[1] = 0.                        # enforce boundary condition (does not work)
...
function calc_agg_y(kpath)
    ...
    ynew = gauss_leg(t -> ces_integrand(t), 150, tmin, tmax) ^ (ϵ / (ϵ - 1.))
end

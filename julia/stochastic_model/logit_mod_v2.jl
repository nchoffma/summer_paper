...
New method for solving: if the full set of FOCs cannot hold, we assume that 
    k* = 0, in which case we solve for c0 and c1 in closed form

The goal (hope) is that now the model should not fail
...
println("*** logit_mod_v2.jl ***")

# Parameters
α = 0.5         # Pr(y>0)
β = 0.95        # discounting
θ_min = 1.0     # works with 3.8
θ_max = 4.0     # looks ok with 4.6
w = 1.2
...
function tax_shoot(Umin)
    ...
    gpath = solve(prob, alg, alg_hints = [:stiff]) 
...
# tspan = (θ_min, θ_max)
# prob = ODEProblem(de_system!, u0, tspan, p)
# gpath_opt = solve(prob, alg, alg_hints = [:stiff])
